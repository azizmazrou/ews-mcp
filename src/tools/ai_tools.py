...
class SemanticSearchEmailsTool(BaseTool):
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # v4.0 — wire the SQLite-backed embedding cache. Falls back
            # gracefully to JSON-only mode if the EWSClient hasn't been
            # configured with a cache (cache_dir kept for that fallback).
            sqlite_cache = getattr(self.ews_client, "sqlite_cache", None)
            model_name = getattr(self.ews_client.config, "ai_embedding_model", None)
